...
{description: <a:yes:802629858365669437> \`Çıkış ban sistemi başarıyla açıldı. Artık sunucudan çıkan herkes banlanacak\`}
{color:303136}{timestamp}{author:$username:$authorAvatar};no]
$onlyIf[$mentionedChannels[1]==;{execute:çıkban}]
  $onlyBotPerms[ban;$customEmoji[yanlis] <a:792704133903810560:806932613322768404> **Kullanıcıları Banlamaya Yetkim Olmadığı İçin Çıkan Kişileri Banlayamam**]
  $onlyIf[$getServerVar[çban]==k;$customEmoji[yanlis] **Çıkış ban sistemi zaten aktif**]
  $elseIf[$message[1]==kapat]
  $setServerVar[çl;]
  $setServerVar[çban;k]
  $title[<a:yes:802629858365669437> **Başarıyla Çıkış Ban Kapatıldı**]
  $description[<a:yes:802629858365669437> **Çıkış ban sistemi başarıyla kapatıldı. Artık sunucudan çıkan kimse banlanmayacak**
...
$author[$username;$authorAvatar]
  $onlyIf[$getServerVar[çban]==a;$customEmoji[yanlis] **Çıkış ban sistemi zaten kapalı**]
  $endElseIf
  $endif
  $onlyIf[$checkCondition[$message[1]==aç]$checkCondition[$message[1]==kapat]!=falsefalse;$customEmoji[yanlis]** Sadece** \`aç\` **Ve Ya** \`kapat\` **Argümanları Kullanılabilir**]
$onlyIf[$message[1]!=;$customEmoji[yanlis]** Sadece** \`aç\` **Ve Ya** \`kapat\` **Argümanları Kullanılabilir**]
  $onlyPerms[ban;$customEmoji[yanlis] Bu Komutu Kullanabilmek İçin **Üyeleri Yasakla** Yetkisine Sahip Olmanız Gerekiyor.]
  `})
...
{color:303136}{timestamp}{author:$username:$authorAvatar};no]
  $onlyBotPerms[ban;$customEmoji[yanlis] **Kullanıcıları Banlamaya Yetkim Olmadığı İçin Çıkan Kişileri Banlayamam**]
  $onlyIf[$getServerVar[çban]==k;$customEmoji[yanlis] **Çıkış Ban Sistemi Zaten Aktif**]`
})
...
bot.leaveCommand({
  ...
  code: `$channelSendMessage[$getServerVar[çl];{title:**Çıkış Ban!**}{description:**<@$authorID> (**\`$userTag[$authorID] - $authorID\`**) ** **Adlı Kullanıcı, Sunucumuzdan Çıkış Yaptı Ve Otomatik Olarak Banlandı!**}{color:BLACK}{timestamp}{author:$username:$userAvatar[$authorID]}{footer:$servername:$replaceText[$serverIcon;null;https://cdn.discordapp.com/attachments/788433498158399488/794897686616080384/indir_1.png;-1]}{thumbnail:$userAvatar[$authorID]}]
    ...
    $ban[$authorid;Sunucudan Çıktığı İçin Banlandı - LEX QUİT BAN SİSTEMİ]
    $onlyBotPerms[ban;]
...
bot.status({
  text: "Lex Şu Anda $serverCount Kadar Sunucya $allMembersCount Kadar Kişiye Hizmet Veriyor!!", 
  type: "WATCHING",
  status: "dnd", 
  time: 12 
})
